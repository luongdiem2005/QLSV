/* EduFee - CẤU HÌNH ĐƠN GIÁ HỌC PHÍ theo loại môn (nối API). Thay ptc_hocphi.js.
 * Bản đồ tới bảng LOAIMONHOC (SoTienMotTinChi). LƯU Ý: ghi loại môn cần quyền
 * ADMIN/PDT (theo phân quyền backend); nếu mở bằng tài khoản PTC, lưu sẽ báo 403.
 * Nếu muốn PTC được sửa đơn giá, đổi WRITE trong catalog.routes.js (backend). */
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function load() {
      ...
      tbody.innerHTML = '';
      if (!loaiList.length) { tbody.innerHTML = '<tr><td colspan="4" class="text-center" style="padding:20px;color:#718096;">Chưa có loại môn.</td></tr>'; return; }
    ...
    if (form) form.addEventListener('submit', async e => {
      ...
      try {
        // PUT cần đủ trường: giữ tên + số tiết/tín chỉ cũ, chỉ đổi đơn giá
